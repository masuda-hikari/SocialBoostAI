...
def auth_headers(client):
    ...
    register_response = client.post(
        ...
        json={
            "email": "youtubetest@example.com",
            "password": "testpassword123",
            "username": "youtubetester",
        },
    )
...
def pro_auth_headers(client):
    ...
    register_response = client.post(
        "/api/v1/auth/register",
        json={
            "email": "youtubeprotest@example.com",
            "password": "testpassword123",
            "username": "youtubeprotester",
        },
    )
    if register_response.status_code not in [201, 409]:
        pytest.fail(f"登録失敗: {register_response.json()}")
    ...
    db = _TestingSessionLocal()
    try:
        user = db.query(User).filter(User.email == "youtubeprotest@example.com").first()
        if user:
            user.role = "pro"
            db.commit()
    finally:
...
class TestYouTubeAnalysisAccess:
    """YouTube分析アクセス制御のテスト"""

    def test_free_user_cannot_access(self, client, auth_headers):
        """Freeプランユーザーはアクセスできない"""
        response = client.get("/api/v1/youtube/analysis", headers=auth_headers)
        assert response.status_code == 403
        assert "Proプラン以上" in response.json()["detail"]

    def test_pro_user_can_access(self, client, pro_auth_headers):
        """Proプランユーザーはアクセスできる"""
        response = client.get("/api/v1/youtube/analysis", headers=pro_auth_headers)
        assert response.status_code == 200


class TestYouTubeAnalysisCreate:
    """YouTube分析作成のテスト"""
    ...
    def test_create_analysis(self, client, pro_auth_headers):
        ...
        assert "total_shorts" in data["summary"]
...
class TestYouTubeAnalysisDetail:
    """YouTube分析詳細のテスト"""
    ...
    def test_get_analysis_detail(self, client, pro_auth_headers):
        ...
        # 詳細取得
        response = client.get(
            f"/api/v1/youtube/analysis/{analysis_id}",
            headers=pro_auth_headers,
        )
        assert response.status_code == 200
        data = response.json()
        ...
        assert "shorts_vs_video" in data

    def test_get_nonexistent_analysis(self, client, pro_auth_headers):
        """存在しない分析は404"""
        response = client.get(
            "/api/v1/youtube/analysis/nonexistent-id",
            headers=pro_auth_headers,
        )
        assert response.status_code == 404


class TestYouTubeAnalysisDelete:
    """YouTube分析削除のテスト"""
